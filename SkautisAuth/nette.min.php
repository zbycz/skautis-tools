...
count(self::$vars)-1;}}define('NETTE',TRUE);define('NETTE_DIR',dirname(__FILE__));define('NETTE_VERSION_ID',20003);define('NETTE_PACKAGE','PHP 5.2 prefixed');interface
IBarPanel{function
getTab();function
getPanel();}interface
IPresenter{function
...
NObjectMixin::callStatic(__CLASS__,$name,$args);}static
function
extensionMethod($name,$callback=NULL){if(strpos($name,'::')===FALSE){$class=__CLASS__;}else{list($class,$name)=explode('::',$name);}$class=new
...
function
load(){if(func_num_args()>1){self::$vars=func_get_arg(1);if(self::$vars===TRUE){return include_once func_get_arg(0);}extract(self::$vars);}return include func_get_arg(0);}}abstract
class
NAutoLoader
extends
NObject{static
...
self;$parts=explode(' ',$name,2);$el->setName($parts[0]);if(is_array($attrs)){$el->attrs=$attrs;}elseif($attrs!==NULL){$el->setText($attrs);}if(isset($parts[1])){foreach(NStrings::matchAll($parts[1].' ','#([a-z0-9:-]+)(?:=(["\'])?(.*?)(?(2)\\2|\s))?#i')as$m){$el->attrs[$m[1]]=isset($m[3])?$m[3]:TRUE;}}return$el;}final
function
...
new
InvalidArgumentException('Email address must be a string.');}self::$email=$email;}if(!defined('E_DEPRECATED')){define('E_DEPRECATED',8192);}if(!defined('E_USER_WARNING')){define('E_USER_WARNING',16384);}if(!self::$enabled){register_shutdown_function(array(__CLASS__,'_shutdownHandler'));set_exception_handler(array(__CLASS__,'_exceptionHandler'));set_error_handler(array(__CLASS__,'_errorHandler'));self::$enabled=TRUE;}}static
function
isEnabled(){return
self::$enabled;}static
function
...
function
initializePanel(NApplication$application){NDebugger::$blueScreen->addPanel(create_function('$e','extract(NCFix::$vars['.NCFix::uses(array('application'=>$application)).'], EXTR_REFS);
			return $e ? NULL : array(
...
__construct(NDIContainer$context){$this->context=$context;}final
function
...
new
NRedirectResponse($url,IHttpResponse::S301_MOVED_PERMANENTLY);}}$params=$request->getParameters();if(!isset($params['callback'])){return;}$params['presenter']=$this;$method=callback($params['callback'])->toReflection();$response=$method->invokeArgs(NPresenterComponentReflection::combineArgs($method,$params));if(is_string($response)){$response=array($response,array());}if(is_array($response)){if($response[0]instanceof
...
__construct($baseDir,NDIContainer$context){$this->baseDir=$baseDir;$this->context=$context;}function
...
getPresenterClass(&$name){if(isset($this->cache[$name])){list($class,$name)=$this->cache[$name];return$class;}if(!is_string($name)||!NStrings::match($name,"#^[a-zA-Z\x7f-\xff][a-zA-Z0-9\x7f-\xff:]*$#")){throw
new
...
new
InvalidStateException("Cannot modify a frozen object $class.");}}}final
class
NPresenterRequest
extends
...
__construct($url,$code=IHttpResponse::S302_FOUND){$this->url=(string)$url;$this->code=(int)$code;}final
function
...
__sleep(){throw
new
NotImplementedException('Object serialization is not supported by class '.get_class($this));}final
function
__wakeup(){throw
new
...
new
UnexpectedValueException("Method $class::$method() did not return or create the desired component.");}return$component;}}final
function
...
new
InvalidStateException("Persistent parameter must be scalar or array, $class::\$$nm is ".gettype($val));}else{if(isset($meta['def'])){settype($val,gettype($meta['def']));if($val===$meta['def']){$val=NULL;}}else{if((string)$val===''){$val=NULL;}}$params[$nm]=$val;}}}final
function
...
function
getPersistentParams(){$arg=func_get_arg(0);$rc=new
NClassReflection($arg);$params=array();foreach($rc->getProperties(ReflectionProperty::IS_PUBLIC)as$rp){if(!$rp->isStatic()&&$rp->hasAnnotation('persistent')){$params[]=$rp->getName();}}return$params;}function
...
new
InvalidStateException('Form is not anchored and therefore can not determine whether it was submitted.');}$this->httpData=$this->receiveHttpData();}return$this->httpData;}function
...
function
receiveHttpData(){$httpRequest=$this->getHttpRequest();if(strcasecmp($this->getMethod(),$httpRequest->getMethod())){return;}if($httpRequest->isMethod('post')){$data=NArrays::mergeTree($httpRequest->getPost(),$httpRequest->getFiles());}else{$data=$httpRequest->getQuery();}if($tracker=$this->getComponent(self::TRACKER_ID,FALSE)){if(!isset($data[self::TRACKER_ID])||$data[self::TRACKER_ID]!==$tracker->getValue()){return;}}return$data;}function
...
__toString(){try{return$this->getRenderer()->render($this);}catch(Exception$e){if(func_get_args()&&func_get_arg(0)){throw$e;}else{NDebugger::toStringException($e);}}}protected
function
...
getParameters(){return$this->params;}function
__toString(){try{return$this->component->link($this->destination,$this->params);}catch(Exception$e){NDebugger::toStringException($e);}}}class
NMultiplier
extends
...
__construct(NDIContainer$context){$this->context=$context;if($this->invalidLinkMode===NULL){$this->invalidLinkMode=$context->parameters['productionMode']?self::INVALID_LINK_SILENT:self::INVALID_LINK_WARNING;}}final
function
...
changeAction($action){if(is_string($action)&&NStrings::match($action,'#^[a-zA-Z0-9][a-zA-Z0-9_\x7f-\xff]*$#')){$this->action=$action;$this->view=$action;}else{$this->error('Action name is not alphanumeric string.');}}final
function
...
NForwardResponse($destination));}elseif(!is_array($args)){$args=func_get_args();array_shift($args);}$this->createRequest($this,$destination,$args,'forward');$this->sendResponse(new
...
NPresenterRequest($presenter,NPresenterRequest::FORWARD,$args,array(),array());$this->lastCreatedRequestFlag=array('current'=>$current);if($mode==='forward'){return;}$url=$router->constructUrl($this->lastCreatedRequest,$refUrl);if($url===NULL){unset($args[self::ACTION_KEY]);$params=urldecode(http_build_query($args,NULL,', '));throw
new
NInvalidLinkException("No route for $presenter:$action($params)");}if($mode==='link'&&$scheme===FALSE&&!$this->absoluteUrls){$hostUrl=$refUrl->getHostUrl();if(strncmp($url,$hostUrl,strlen($hostUrl))===0){$url=substr($url,strlen($hostUrl));}}return$url.$fragment;}private
static
function
...
new
NInvalidLinkException("Invalid value for parameter '$name', expected array.");}}elseif($param->getClass()||is_object($val)){}elseif(!is_scalar($val)){throw
new
NInvalidLinkException("Invalid value for parameter '$name', expected scalar.");}elseif($def===NULL){if((string)$val===''){$args[$name]=NULL;}continue;}else{settype($args[$name],gettype($def));if((string)$args[$name]!==(string)$val){throw
new
NInvalidLinkException("Invalid value for parameter '$name', expected ".gettype($def).".");}}if($args[$name]===$def){$args[$name]=NULL;}}if(array_key_exists($i,$args)){$method=$rm->getName();throw
new
...
function
getPersistentComponents(){$arg=func_get_arg(0);return(array)NClassReflection::from($arg)->getAnnotation('persistent');}private
function
...
getFlashSession(){if(empty($this->params[self::FLASH_KEY])){$this->params[self::FLASH_KEY]=NStrings::random(4);}return$this->getSession('Nette.Application.Flash/'.$this->params[self::FLASH_KEY]);}final
function
...
new
NBadRequestException("Invalid value for parameter '$name', expected array.");}}elseif($param->getClass()||is_object($val)){}else{if(!is_scalar($val)){throw
new
NBadRequestException("Invalid value for parameter '$name', expected scalar.");}if($def!==NULL){settype($val,gettype($def));if(($val===FALSE?'0':(string)$val)!==(string)$args[$name]){throw
new
NBadRequestException("Invalid value for parameter '$name', expected ".gettype($def).".");}}}$res[$i++]=$val;}}return$res;}}class
NCache
extends
NObject
implements
...
NCallback||$data
instanceof
Closure){$this->storage->lock($key);$data=callback($data)->invokeArgs(array(&$dp));}if($data===NULL){$this->storage->remove($key);}else{$this->storage->write($key,$data,$this->completeDependencies($dp,$data));return$data;}}private
function
...
call($function){$key=func_get_args();return$this->load($key,create_function('','extract(NCFix::$vars['.NCFix::uses(array('function'=>$function,'key'=>$key)).'], EXTR_REFS);
			array_shift($key);
			return callback($function)->invokeArgs($key);
		'));}function
wrap($function,array$dp=NULL){$cache=$this;return
create_function('','extract(NCFix::$vars['.NCFix::uses(array('cache'=>$cache,'function'=>$function,'dp'=>$dp)).'], EXTR_REFS);
...
new
InvalidStateException('Output cache has already been saved.');}$this->cache->save($this->key,ob_get_flush(),(array)$dp+(array)$this->dependencies);$this->cache=NULL;}}class
NDevNullStorage
extends
NObject
implements
...
addServer($host='localhost',$port=11211,$timeout=1){NDebugger::tryError();$this->memcache->addServer($host,$port,TRUE,1,$timeout);if(NDebugger::catchError($e)){throw
new
InvalidStateException('Memcache::addServer(): '.$e->getMessage(),0,$e);}}function
...
__construct($cb,$m=NULL){if($m!==NULL){$cb=array($cb,$m);}if(PHP_VERSION_ID<50202&&is_string($cb)&&strpos($cb,'::')){$cb=explode('::',$cb,2);}elseif(is_object($cb)&&!$cb
instanceof
...
DateTimeZone($this->getTimezone()->getName()));}}final
class
NEnvironment{const
...
new
NCache(self::getContext()->cacheStorage,$namespace);}static
function
...
ob_get_clean();}function
__toString(){try{return$this->toString();}catch(Exception$e){NDebugger::toStringException($e);}}function
...
load($file){NDebugger::tryError();$ini=parse_ini_file($file,TRUE);if(NDebugger::catchError($e)){throw
new
InvalidStateException('parse_ini_file(): '.$e->getMessage(),0,$e);}$data=array();foreach($ini
...
new
InvalidStateException("Replacing operator is available only for arrays, item '$key' is not array.");}$key=substr($key,0,-1);$val[NConfigHelpers::EXTENDS_KEY]=NConfigHelpers::OVERWRITE;}elseif(preg_match('#^(\S+)\s+'.self::INHERITING_SEPARATOR.'\s+(\S+)$#',$key,$matches)){if(!is_array($val)&&$val!==NULL){throw
new
...
new
InvalidStateException("Found sections '$extra' in configuration, but corresponding extensions are missing.");}foreach($this->extensions
...
implode("\n\n\n",$classes);}static
function
parseServices(NDIContainerBuilder$container,array$config,$namespace=NULL){$services=isset($config['services'])?$config['services']:array();$factories=isset($config['factories'])?$config['factories']:array();if($tmp=array_intersect_key($services,$factories)){$tmp=implode("', '",array_keys($tmp));throw
new
NServiceCreationException("It is not allowed to use services and factories with the same names: '$tmp'.");}$all=$services+$factories;uasort($all,create_function('$a, $b','
			return strcmp(NConfigHelpers::isInheriting($a), NConfigHelpers::isInheriting($b));
		'));foreach($all
as$name=>$def){$shared=array_key_exists($name,$services);$name=($namespace?$namespace.'.':'').$name;if(($parent=NConfigHelpers::takeParent($def))&&$parent!==$name){$container->removeDefinition($name);$definition=$container->addDefinition($name);if($parent!==NConfigHelpers::OVERWRITE){foreach($container->getDefinition($parent)as$k=>$v){$definition->$k=$v;}}}elseif($container->hasDefinition($name)){$definition=$container->getDefinition($name);if($definition->shared!==$shared){throw
new
...
NDIStatement('NRoutingDebugger')));}if(empty($config['mailer']['smtp'])){$container->addDefinition($this->prefix('mailer'))->setClass('NSendmailMailer');}else{NValidators::assertField($config,'mailer','array');$container->addDefinition($this->prefix('mailer'))->setClass('NSmtpMailer',array($config['mailer']));}$container->addDefinition($this->prefix('mail'))->setClass('NMail')->addSetup('setMailer')->setShared(FALSE);$container->addDefinition($this->prefix('basicForm'))->setClass('NForm')->setShared(FALSE);$latte=$container->addDefinition($this->prefix('latte'))->setClass('NLatteFilter')->setShared(FALSE);if(empty($config['xhtml'])){$latte->addSetup('$service->getCompiler()->defaultContentType = ?',NLatteCompiler::CONTENT_HTML);}$container->addDefinition($this->prefix('template'))->setClass('NFileTemplate')->addSetup('registerFilter',array($latte))->addSetup('registerHelperLoader',array('NTemplateHelpers::loader'))->setShared(FALSE);$container->addDefinition($this->prefix('database'))->setClass('NDINestedAccessor',array('@container',$this->prefix('database')));$autowired=TRUE;foreach((array)$config['database']as$name=>$info){if(!is_array($info)){continue;}$info+=$this->databaseDefaults+array('autowired'=>$autowired);$autowired=FALSE;foreach((array)$info['options']as$key=>$value){unset($info['options'][$key]);$info['options'][constant($key)]=$value;}$connection=$container->addDefinition($this->prefix("database.$name"))->setClass('NConnection',array($info['dsn'],$info['user'],$info['password'],$info['options']))->setAutowired($info['autowired'])->addSetup('setCacheStorage')->addSetup('NDebugger::$blueScreen->addPanel(?)',array('NDatabasePanel::renderException'));if($info['reflection']){$connection->addSetup('setDatabaseReflection',is_string($info['reflection'])?array(new
NDIStatement(preg_match('#^[a-z]+$#',$info['reflection'])?'Nette\Database\Reflection\\'.ucfirst($info['reflection']).'Reflection':$info['reflection'])):NConfigCompiler::filterArguments(array($info['reflection'])));}if(!$container->parameters['productionMode']&&$info['debugger']){$panel=$container->addDefinition($this->prefix("database.{$name}ConnectionPanel"))->setClass('NDatabasePanel')->setAutowired(FALSE)->addSetup('$explain',!empty($info['explain']))->addSetup('NDebugger::$bar->addPanel(?)',array('@self'));$connection->addSetup('$service->onQuery[] = ?',array(array($panel,'logQuery')));}}}function
...
instanceof
PDOException&&isset($e->queryString)){return
array('tab'=>'SQL','panel'=>NDatabaseHelpers::dumpSql($e->queryString));}}function
...
getTables(){throw
new
NNotImplementedException;}function
...
new
FileNotFoundException("Cannot open file '$file'.");}$count=0;$sql='';while(!feof($handle)){$s=fgets($handle);$sql.=$s;if(substr(rtrim($s),-1)===';'){$connection->exec($sql);$sql='';$count++;}}if($sql!==''){$connection->exec($sql);$count++;}fclose($handle);return$count;}}class
...
NObject
implements
IReflection{protected$cache;protected$cacheStorage;protected$connection;protected$structure=array('primary'=>array(),'hasMany'=>array(),'belongsTo'=>array());function
...
NCache($this->cacheStorage,'Nette.Database.'.md5($connection->getDsn()));$this->structure=($tmp=$this->cache->load('structure'))?$tmp:$this->structure;}}function
...
new
PDOException("No reference found for \${$table}->related({$key}).");}$this->reloadAllForeignKeys();return$this->getHasManyReference($table,$key,FALSE);}function
...
new
PDOException("No reference found for \${$table}->{$key}.");}$this->reloadForeignKeys($table);return$this->getBelongsToReference($table,$key,FALSE);}protected
function
reloadAllForeignKeys(){foreach($this->connection->getSupplementalDriver()->getTables()as$table){if($table['view']==FALSE){$this->reloadForeignKeys($table['name']);}}foreach(array_keys($this->structure['hasMany'])as$table){uksort($this->structure['hasMany'][$table],create_function('$a, $b','
				return strlen($a) - strlen($b);
			'));}}protected
function
reloadForeignKeys($table){foreach($this->connection->getSupplementalDriver()->getForeignKeys($table)as$row){$this->structure['belongsTo'][$table][$row['local']]=$row['table'];$this->structure['hasMany'][$row['table']][$table]=$row['local'];}if(isset($this->structure['belongsTo'][$table])){uksort($this->structure['belongsTo'][$table],create_function('$a, $b','
				return strlen($a) - strlen($b);
			'));}}}class
NRow
extends
NArrayHash{function
__construct(NStatement$statement){$statement->normalizeRow($this);}function
...
process($sql,$params){$this->params=$params;$this->counter=0;$this->remaining=array();$cmd=strtoupper(substr(ltrim($sql),0,6));$this->arrayMode=$cmd==='INSERT'||$cmd==='REPLAC'?'values':'assoc';$sql=NStrings::replace($sql,'~\'.*?\'|".*?"|\?~s',array($this,'callback'));while($this->counter<count($params)){$sql.=' '.$this->formatValue($params[$this->counter++]);}return
...
function
detectColumnTypes(){if($this->types===NULL){$this->types=array();if($this->connection->getSupplementalDriver()->isSupported(ISupplementalDriver::META)){$col=0;while($meta=$this->getColumnMeta($col++)){if(isset($meta['native_type'])){$this->types[$meta['name']]=NDatabaseHelpers::detectType($meta['native_type']);}}}}return$this->types;}function
...
NObject
implements
IteratorAggregate,ArrayAccess{private$table;private$data;private$modified=array();function
__construct(array$data,NTableSelection$table){$this->data=$data;$this->table=$table;}function
getTable(){return$this->table;}function
__toString(){try{return(string)$this->getPrimary();}catch(Exception$e){NDebugger::toStringException($e);}}function
toArray(){$this->access(NULL);return$this->data;}function
getPrimary(){if(!isset($this->data[$this->table->getPrimary()])){throw
new
NotSupportedException("Table {$this->table->getName()} does not have any primary key.");}return$this[$this->table->getPrimary()];}function
...
NObject
implements
Iterator,ArrayAccess,Countable{protected$connection;protected$name;protected$primary;protected$rows;protected$data;protected$select=array();protected$where=array();protected$conditions=array();protected$parameters=array();protected$order=array();protected$limit=NULL;protected$offset=NULL;protected$group='';protected$having='';protected$checkReferenceNewKeys=FALSE;protected$referenced=array();protected$referencing=array();protected$accessed;protected$prevAccessed;protected$keys=array();protected$delimitedName;protected$delimitedPrimary;function
__construct($table,NConnection$connection){$this->name=$table;$this->connection=$connection;$this->primary=$connection->getDatabaseReflection()->getPrimary($table);$this->delimitedName=$this->tryDelimite($this->name);$this->delimitedPrimary=$connection->getSupplementalDriver()->delimite($this->primary);}function
...
instanceof
NTableSelection){$clone=clone$parameters;if(!$clone->select){$clone->select=array($clone->primary);}if($this->connection->getAttribute(PDO::ATTR_DRIVER_NAME)!=='mysql'){$condition.=' IN ('.$clone->getSql().')';}else{$in=array();foreach($clone
...
NTableSelection($this->name,$this->connection);$selection->where=$this->where;$selection->parameters=$this->parameters;$selection->conditions=$this->conditions;$selection->select($function);foreach($selection->fetch()as$val){return$val;}}function
...
preg_replace_callback('#(?<=[^\w`"\[]|^)[a-z_][a-z0-9_]*(?=[^\w`"(\]]|$)#i',create_function('$m','extract(NCFix::$vars['.NCFix::uses(array('driver'=>$driver)).'], EXTR_REFS);
...
instanceof
Traversable){$data=iterator_to_array($data);}$return=$this->connection->query("INSERT INTO $this->delimitedName",$data);if(!is_array($data)){return$return->rowCount();}$this->checkReferenceNewKeys=TRUE;if(!isset($data[$this->primary])&&($id=$this->connection->lastInsertId())){$data[$this->primary]=$id;return$this->rows[$id]=new
NTableRow($data,$this);}else{return
new
...
NTableRow?$row[$column]->getPrimary():$row[$column];$keys[$key]=TRUE;}if($referenced!==NULL&&$keys===array_keys($this->rows)){$this->checkReferenceNewKeys=FALSE;return$referenced;}if($keys){$referenced=new
NTableSelection($table,$this->connection);$referenced->where($table.'.'.$referenced->primary,array_keys($keys));}else{$referenced=array();}}return$referenced;}function
...
NGroupedTableSelection($table,$this,$column);$referencing->where("$table.$column",array_keys((array)$this->rows));}return$referencing->setActive($active);}function
...
extends
NTableSelection{protected$refTable;protected$column;protected$delimitedColumn;protected$active;protected$referencing;protected$aggregation=array();function
__construct($name,NTableSelection$refTable,$column){parent::__construct($name,$refTable->connection);$this->refTable=$refTable;$this->column=$column;$this->delimitedColumn=$this->connection->getSupplementalDriver()->delimite($this->column);}function
...
NTableSelection($this->name,$this->connection);$selection->where=$this->where;$selection->parameters=$this->parameters;$selection->conditions=$this->conditions;$selection->select($function);$selection->select("{$this->name}.{$this->column}");$selection->group("{$this->name}.{$this->column}");foreach($selection
...
as$key=>$row){$ref=&$referencing[$row[$this->column]];$skip=&$offset[$row[$this->column]];if($limit===NULL||$rows<=1||(count($ref)<$limit&&$skip>=$this->offset)){$ref[$key]=$row;}else{unset($this->rows[$key]);}$skip++;unset($ref,$skip);}}$this->data=&$referencing[$this->active];if($this->data===NULL){$this->data=array();}else{reset($this->data);}}}class
NDIContainer
extends
NFreezableObject
implements
...
Closure&&!$service
instanceof
NCallback){$this->registry[$name]=$service;$this->meta[$name]=$meta;return$this;}elseif(!is_string($service)||strpos($service,':')!==FALSE||$service[0]==="\0"){$service=callback($service);}$this->factories[$name]=array($service);$this->registry[$name]=&$this->factories[$name][1];$this->meta[$name]=$meta;return$this;}function
...
new
InvalidArgumentException("Class $class is not instantiable.");}elseif($constructor=$rc->getConstructor()){return$rc->newInstanceArgs(NDIHelpers::autowireArguments($constructor,$args,$this));}elseif($args){throw
new
InvalidArgumentException("Unable to pass arguments, class $class has no constructor.");}return
new$class;}function
callMethod($function,array$args=array()){$callback=callback($function);return$callback->invokeArgs(NDIHelpers::autowireArguments($callback->toReflection(),$args,$this));}function
expand($s){return
...
function
getMethodName($name,$isService=TRUE){$uname=ucfirst($name);return($isService?'createService':'create').($name===$uname?'__':'').str_replace('.','__',$uname);}}class
NDIContainerBuilder
extends
NObject{const
...
new
NServiceCreationException("Class and factory are missing in service '$name' definition.");}}$this->classes=FALSE;foreach($this->definitions
...
new
InvalidStateException("Class $def->class has not been found.");}$def->class=NClassReflection::from($def->class)->getName();if($def->autowired){foreach(class_parents($def->class)+class_implements($def->class)+array($def->class)as$parent){$this->classes[strtolower($parent)][]=$name;}}}foreach($this->classes
...
new
NServiceCreationException("Unable resolve class name for service '$name'.");}$factory[0]=$this->resolveClass($service,$recursive);if(!$factory[0]){return;}}$factory=callback($factory);if(!$factory->isCallable()){throw
new
InvalidStateException("Factory '$factory' is not callable.");}try{$reflection=$factory->toReflection();$def->class=preg_replace('#[|\s].*#','',$reflection->getAnnotation('return'));if($def->class&&!class_exists($def->class)&&$def->class[0]!=='\\'&&$reflection
instanceof
ReflectionMethod){}}catch(ReflectionException$e){}}elseif($service=$this->getServiceName($factory)){if(NStrings::contains($service,'\\')){$service=ltrim($service,'\\');$def->autowired=FALSE;return$def->class=$service;}if($this->definitions[$service]->shared){$def->autowired=FALSE;}return$def->class=$this->resolveClass($service,$recursive);}else{return$def->class=$factory;}}function
...
as$name=>$def){try{$type=($tmp=$def->class)?$tmp:'object';$methodName=NDIContainer::getMethodName($name,$def->shared);if(!NPhpHelpers::isIdentifier($methodName)){throw
new
...
as$k=>$v){$params[]=preg_replace('#\w+$#','\$$0',(is_int($k)?$v:$k)).(is_int($k)?'':' = '.NPhpHelpers::dump($v));}$rm=new
...
new
InvalidArgumentException('Circular reference detected for variables: '.implode(', ',array_keys($recursive)).'.');}else{$val=NArrays::get($params,explode('.',$part));if($recursive){$val=self::expand($val,$params,(is_array($recursive)?$recursive:array())+array($part=>1));}if(strlen($part)+2===strlen($var)){return$val;}if(!is_scalar($val)){throw
new
...
new
InvalidArgumentException("No service of type {$class} found");}}else{if($container
instanceof
...
new
InvalidArgumentException("$parameter is missing.");}}while(array_key_exists(++$num,$arguments)){$res[$num]=$arguments[$num];unset($arguments[$num]);$optCount=0;}if($arguments){throw
new
InvalidArgumentException("Unable to pass specified arguments to $method.");}return$optCount?array_slice($res,0,-$optCount):$res;}}class
NDINestedAccessor
extends
...
NDIStatement($target,$args);return$this;}function
...
loadHttpData(){$path=$this->getHtmlName();$path=explode('[',strtr(str_replace(']','',strpos($path,'[')===FALSE?$path.'.x':substr($path,0,-2)),'.','_'));$this->setValue(NArrays::get($this->getForm()->getHttpData(),$path,NULL)!==NULL);}}class
NSelectBox
extends
...
getControl(){$control=parent::getControl();if($this->prompt){reset($this->items);$control->data('nette-empty-value',$this->useKeys?key($this->items):current($this->items));}$selected=$this->getValue();$selected=is_array($selected)?array_flip($selected):array($selected=>TRUE);$option=NHtml::el('option');foreach($this->items
...
instanceof
NHtml){$dest->add((string)$value2->selected(isset($selected[$key2])));}else{$key2=$this->useKeys?$key2:$value2;$value2=$this->translate((string)$value2);$dest->add((string)$option->value($key2)->selected(isset($selected[$key2]))->setText($value2));}}}return$control;}}class
NMultiSelectBox
extends
NSelectBox{function
...
function
validatePattern(NTextBase$control,$pattern){return(bool)NStrings::match($control->getValue(),"\x01^($pattern)$\x01u");}static
function
...
sanitize($value){if($this->control->maxlength&&NStrings::length($value)>$this->control->maxlength){$value=NStrings::substring($value,0,$this->control->maxlength);}return
NStrings::trim(strtr($value,"\r\n",'  '));}function
...
move($dest){@mkdir(dirname($dest),0777,TRUE);if(substr(PHP_OS,0,3)==='WIN'){@unlink($dest);}if(!call_user_func(is_uploaded_file($this->tmpName)?'move_uploaded_file':'rename',$this->tmpName,$dest)){throw
new
...
NArrays::get($this->files,$args,NULL);}final
function
...
new
NHttpRequest($url,$query,$post,$files,$cookies,$headers,isset($_SERVER['REQUEST_METHOD'])?$_SERVER['REQUEST_METHOD']:NULL,isset($_SERVER['REMOTE_ADDR'])?$_SERVER['REMOTE_ADDR']:NULL,isset($_SERVER['REMOTE_HOST'])?$_SERVER['REMOTE_HOST']:NULL);}}final
class
NHttpResponse
extends
NObject
...
__construct(IHttpRequest$request,IHttpResponse$response){$this->request=$request;$this->response=$response;}function
start(){if(self::$started){return;}$this->configure($this->options);NDebugger::tryError();session_start();if(NDebugger::catchError($e)&&!session_id()){@session_write_close();throw
new
InvalidStateException('session_start(): '.$e->getMessage(),0,$e);}self::$started=TRUE;unset($_SESSION['__NT'],$_SESSION['__NS'],$_SESSION['__NM']);$nf=&$_SESSION['__NF'];if(empty($nf)){$nf=array('C'=>0);}else{$nf['C']++;}$nfTime=&$nf['Time'];$time=time();if($time-$nfTime>self::REGENERATE_INTERVAL){$this->regenerated=$this->regenerated||isset($nfTime);$nfTime=$time;}$browserKey=$this->request->getCookie('nette-browser');if(!$browserKey){$browserKey=NStrings::random();}$browserClosed=!isset($nf['B'])||$nf['B']!==$browserKey;$nf['B']=$browserKey;$this->sendCookie();if(isset($nf['META'])){$now=time();foreach($nf['META']as$section=>$metadata){if(is_array($metadata)){foreach($metadata
...
new
InvalidStateException("Cannot regenerate session ID after HTTP headers have been sent".($file?" (output started at $file:$line).":"."));}session_regenerate_id(TRUE);session_write_close();$backup=$_SESSION;session_start();$_SESSION=$backup;}$this->regenerated=TRUE;}function
getId(){return
session_id();}function
...
function
sendCookie(){$cookie=$this->getCookieParameters();$this->response->setCookie(session_name(),session_id(),$cookie['lifetime']?$cookie['lifetime']+time():0,$cookie['path'],$cookie['domain'],$cookie['secure'],$cookie['httponly'])->setCookie('nette-browser',$_SESSION['__NF']['B'],NHttpResponse::BROWSER,$cookie['path'],$cookie['domain']);}}final
class
NSessionSection
extends
NObject
...
NHtmlNode($token->name);}}while(strcasecmp($htmlNode->name,$token->name));$this->htmlNodes[]=$htmlNode;$htmlNode->closing=TRUE;$htmlNode->offset=strlen($this->output);$this->setContext(NULL);}elseif($token->text==='<!--'){$this->setContext(self::CONTEXT_COMMENT);}else{$this->htmlNodes[]=$htmlNode=new
...
writeAttrsMacro($code,NHtmlNode$htmlNode){$attrs=$htmlNode->macroAttrs;$left=$right=array();$attrCode='';foreach($this->macros
...
function
createCache(ICacheStorage$cacheStorage,$key,&$parents,array$args=NULL){if($args){if(array_key_exists('if',$args)&&!$args['if']){return$parents[]=(object)NULL;}$key=array_merge(array($key),array_intersect_key($args,range(0,count($args))));}if($parents){end($parents)->dependencies[NCache::ITEMS][]=$key;}$cache=new
...
self($compiler);$me->addMacro('if',array($me,'macroIf'),array($me,'macroEndIf'));$me->addMacro('elseif','elseif (%node.args):');$me->addMacro('else',array($me,'macroElse'));$me->addMacro('ifset','if (isset(%node.args)):','endif');$me->addMacro('elseifset','elseif (isset(%node.args)):');$me->addMacro('foreach','',array($me,'macroEndForeach'));$me->addMacro('for','for (%node.args):','endfor');$me->addMacro('while','while (%node.args):','endwhile');$me->addMacro('continueIf','if (%node.args) continue');$me->addMacro('breakIf','if (%node.args) break');$me->addMacro('first','if ($iterator->isFirst(%node.args)):','endif');$me->addMacro('last','if ($iterator->isLast(%node.args)):','endif');$me->addMacro('sep','if (!$iterator->isLast(%node.args)):','endif');$me->addMacro('var',array($me,'macroVar'));$me->addMacro('assign',array($me,'macroVar'));$me->addMacro('default',array($me,'macroVar'));$me->addMacro('dump',array($me,'macroDump'));$me->addMacro('debugbreak',array($me,'macroDebugbreak'));$me->addMacro('l','?>{<?php');$me->addMacro('r','?>}<?php');$me->addMacro('_',array($me,'macroTranslate'),array($me,'macroTranslate'));$me->addMacro('=',array($me,'macroExpr'));$me->addMacro('?',array($me,'macroExpr'));$me->addMacro('capture',array($me,'macroCapture'),array($me,'macroCaptureEnd'));$me->addMacro('include',array($me,'macroInclude'));$me->addMacro('use',array($me,'macroUse'));$me->addMacro('class',NULL,NULL,array($me,'macroClass'));$me->addMacro('attr',array($me,'macroOldAttr'),'',array($me,'macroAttr'));$me->addMacro('href',NULL);}function
finalize(){return
...
new
NCompileException('Missing condition in {if} macro.');}return$writer->write('if (%node.args) '.(isset($node->data->else)?'{ ob_end_clean(); ob_end_flush(); }':'ob_end_flush();').' else '.(isset($node->data->else)?'{ $_else = ob_get_contents(); ob_end_clean(); ob_end_clean(); echo $_else; }':'ob_end_clean();'));}return'endif';}function
...
macroUse(NMacroNode$node,NPhpWriter$writer){callback($node->tokenizer->fetchWord(),'install')->invoke($this->getCompiler())->initialize();}function
...
function
initRuntime(ITemplate$template,$templateId){if(isset($template->_l)){$local=$template->_l;unset($template->_l);}else{$local=(object)NULL;}$local->templates[$templateId]=$template;if(!isset($template->_g)){$template->_g=(object)NULL;}return
...
macroLabel(NMacroNode$node,NPhpWriter$writer){$cmd='if ($_label = $_form[%node.word]->getLabel()) echo $_label->addAttributes(%node.array)';if($node->isEmpty=(substr($node->args,-1)==='/')){$node->setArgs(substr($node->args,0,-1));return$writer->write($cmd);}else{return$writer->write($cmd.'->startTag()');}}function
macroAttrInput(NMacroNode$node,NPhpWriter$writer){if($node->htmlNode->attrs){$reset=array_fill_keys(array_keys($node->htmlNode->attrs),NULL);return$writer->write('echo $_form[%node.word]->getControl()->addAttributes(%var)->attributes()',$reset);}return$writer->write('echo $_form[%node.word]->getControl()->attributes()');}static
function
renderFormBegin(NForm$form,array$attrs){$el=$form->getElementPrototype();$el->action=(string)$el->action;$el=clone$el;if(strcasecmp($form->getMethod(),'get')===0){list($el->action)=explode('?',$el->action,2);}echo$el->addAttributes($attrs)->startTag();}static
function
renderFormEnd(NForm$form){$s='';if(strcasecmp($form->getMethod(),'get')===0){$url=explode('?',$form->getElementPrototype()->action,2);if(isset($url[1])){foreach(preg_split('#[;&]#',$url[1])as$param){$parts=explode('=',$param,2);$name=urldecode($parts[0]);if(!isset($form[$name])){$s.=NHtml::el('input',array('type'=>'hidden','name'=>$name,'value'=>urldecode($parts[1])));}}}}foreach($form->getComponents(TRUE,'NHiddenField')as$control){if(!$control->getOption('rendered')){$s.=$control->getControl();}}if(iterator_count($form->getComponents(TRUE,'NTextInput'))<2){$s.='<!--[if IE]><input type=IEbug disabled style="display:none"><![endif]-->';}echo($s?"<div>$s</div>\n":'').$form->getElementPrototype()->endTag()."\n";}}class
NUIMacros
extends
NMacroSet{const
...
function
install(NLatteCompiler$compiler){$me=new
self($compiler);$me->addMacro('include',array($me,'macroInclude'));$me->addMacro('includeblock',array($me,'macroIncludeBlock'));$me->addMacro('extends',array($me,'macroExtends'));$me->addMacro('layout',array($me,'macroExtends'));$me->addMacro('block',array($me,'macroBlock'),array($me,'macroBlockEnd'));$me->addMacro('define',array($me,'macroBlock'),array($me,'macroBlockEnd'));$me->addMacro('snippet',array($me,'macroBlock'),array($me,'macroBlockEnd'));$me->addMacro('ifset',array($me,'macroIfset'),'endif');$me->addMacro('widget',array($me,'macroControl'));$me->addMacro('control',array($me,'macroControl'));$me->addMacro('href',NULL,NULL,create_function('NMacroNode $node, NPhpWriter $writer','extract(NCFix::$vars['.NCFix::uses(array('me'=>$me)).'], EXTR_REFS);
			return \' ?> href="<?php \' . $me->macroLink($node, $writer) . \' ?>"<?php \';
		'));$me->addMacro('plink',array($me,'macroLink'));$me->addMacro('link',array($me,'macroLink'));$me->addMacro('ifCurrent',array($me,'macroIfCurrent'),'endif');$me->addMacro('contentType',array($me,'macroContentType'));$me->addMacro('status',array($me,'macroStatus'));}function
...
FALSE;}$destination=ltrim($destination,'#');if(!NStrings::match($destination,'#^\$?'.self::RE_IDENTIFIER.'$#')){throw
new
NCompileException("Included block name must be alphanumeric string, '$destination' given.");}$parent=$destination==='parent';if($destination==='parent'||$destination==='this'){for($item=$node->parentNode;$item&&$item->name!=='block'&&!isset($item->data->name);$item=$item->parentNode);if(!$item){throw
new
...
new
NCompileException("Missing destination in {extends}");}if(!empty($node->parentNode)){throw
new
NCompileException("{extends} must be placed outside any macro.");}if($this->extends!==NULL){throw
new
NCompileException("Multiple {extends} declarations are not allowed.");}if($node->args==='none'){$this->extends='FALSE';}elseif($node->args==='auto'){$this->extends='$_presenter->findLayoutTemplateFile()';}else{$this->extends=$writer->write('%node.word%node.args');}return;}function
...
new
NCompileException("Missing block name.");}}elseif(!NStrings::match($name,'#^'.self::RE_IDENTIFIER.'$#')){if($node->name==='snippet'){for($parent=$node->parentNode;$parent&&$parent->name!=='snippet';$parent=$parent->parentNode);if(!$parent){throw
new
...
parseMacroTag($tag){$match=NStrings::match($tag,'~^
		...
		()$~isx');if(!$match){return
FALSE;}if($match['name']===''){$match['name']=($tmp=$match['shortname'])?$tmp:'=';if(!$match['noescape']&&substr($match['shortname'],0,1)!=='/'){$match['modifiers'].='|escape';}}return
...
write($mask){$args=func_get_args();array_shift($args);$word=strpos($mask,'%node.word')===FALSE?NULL:$this->argsTokenizer->fetchWord();$me=$this;$mask=NStrings::replace($mask,'#%escape(\(([^()]*+|(?1))+\))#',callback(create_function('$m','extract(NCFix::$vars['.NCFix::uses(array('me'=>$me)).'], EXTR_REFS);
		...
		')));return
NStrings::replace($mask,'#([,+]\s*)?%(node\.word|node\.array|node\.args|var|raw)(\?)?(\s*\+\s*)?()#',callback(create_function('$m','extract(NCFix::$vars['.NCFix::uses(array('me'=>$me,'word'=>$word,'args'=>&$args)).'], EXTR_REFS);
...
NLatteCompiler::CONTEXT_DOUBLE_QUOTED:if($context[1]===NLatteCompiler::CONTENT_JS){$s="NTemplateHelpers::escapeJs($s)";}elseif($context[1]===NLatteCompiler::CONTENT_CSS){$s="NTemplateHelpers::escapeCss($s)";}$quote=$context[0]===NLatteCompiler::CONTEXT_DOUBLE_QUOTED?'':', ENT_QUOTES';return"htmlSpecialChars($s$quote)";case
...
NLatteCompiler::CONTENT_XML:case
NLatteCompiler::CONTENT_JS:case
...
tryLoad($type){$type=ltrim(strtolower($type),'\\');$info=&$this->list[$type];if($this->autoRebuild&&empty($this->checked[$type])&&(is_array($info)?!is_file($info[0]):$info<self::RETRY_LIMIT)){$info=is_int($info)?$info+1:0;$this->checked[$type]=TRUE;if($this->rebuilt){$this->getCache()->save($this->getKey(),$this->list,array(NCache::CONSTS=>'NFramework::REVISION'));}else{$this->rebuild();}}if(isset($info[0])){NLimitedScope::load($info[0],TRUE);if($this->autoRebuild&&!class_exists($type,FALSE)&&!interface_exists($type,FALSE)&&(PHP_VERSION_ID<50400||!trait_exists($type,FALSE))){$info=0;$this->checked[$type]=TRUE;if($this->rebuilt){$this->getCache()->save($this->getKey(),$this->list,array(NCache::CONSTS=>'NFramework::REVISION'));}else{$this->rebuild();}}self::$count++;}}function
...
as$pair){if(is_array($pair)){$this->files[$pair[0]]=$pair[1];}}foreach(array_unique($this->scanDirs)as$dir){$this->scanDirectory($dir);}$this->files=NULL;$dp=array(NCache::CONSTS=>'NFramework::REVISION');return$this->list;}function
...
new
DirectoryNotFoundException("Directory '$val' not found.");}$this->scanDirs[]=$real;}return$this;}private
function
...
InvalidStateException("Ambiguous class '$class' resolution; defined in $file and in ".$this->list[$lClass][0].".");if(PHP_VERSION_ID<50300){NDebugger::_exceptionHandler($e);exit;}else{throw$e;}}$this->list[$lClass]=array($file,$time,$class);$this->files[$file]=$time;}private
function
scanDirectory($dir){if(is_dir($dir)){$ignoreDirs=is_array($this->ignoreDirs)?$this->ignoreDirs:NStrings::split($this->ignoreDirs,'#[,\s]+#');$disallow=array();foreach($ignoreDirs
as$item){if($item=realpath($item)){$disallow[$item]=TRUE;}}$iterator=NFinder::findFiles(is_array($this->acceptFiles)?$this->acceptFiles:NStrings::split($this->acceptFiles,'#[,\s]+#'))->filter(create_function('$file','extract(NCFix::$vars['.NCFix::uses(array('disallow'=>&$disallow)).'], EXTR_REFS);
...
as$class=>$pair){if(is_array($pair)&&$pair[0]===$file){unset($this->list[$class]);}}if($matches=NStrings::match($s,'#//nette'.'loader=(\S*)#')){foreach(explode(',',$matches[1])as$name){$this->addClass($name,$file,$time);}return;}foreach(@token_get_all($s)as$token){if(is_array($token)){switch($token[0]){case
T_COMMENT:case
T_DOC_COMMENT:case
...
toCallback(){return
new
NCallback(array(parent::getDeclaringClass()->getName(),$this->getName()));}function
...
__construct(IUserStorage$storage,NDIContainer$context){$this->storage=$storage;$this->context=$context;}final
function
getStorage(){return$this->storage;}function
...
ob_get_clean();}catch(Exception$e){ob_end_clean();if($args&&$args[0]){throw$e;}else{NDebugger::toStringException($e);}}}function
...
new
InvalidStateException("Filter '$callback' was registered twice.");}$this->filters[]=$callback;return$this;}final
function
...
function
strip($s){return
NStrings::replace($s,'#(</textarea|</pre|</script|^).*?(?=<textarea|<pre|<script|$)#si',callback(create_function('$m','
				return trim(preg_replace("#[ \\t\\r\\n]+#", " ", $m[0]));
			')));}static
function
indent($s,$level=1,$chars="\t"){if($level>=1){$s=NStrings::replace($s,'#<(textarea|pre).*?</\\1#si',callback(create_function('$m','
				return strtr($m[0], " \\t\\r\\n", "\\x1F\\x1E\\x1D\\x1A");
...
function
grep(array$arr,$pattern,$flags=0){NDebugger::tryError();$res=preg_grep($pattern,$arr,$flags);if(NDebugger::catchError($e)||preg_last_error()){throw
new
NRegexpException($e?$e->getMessage():NULL,$e?NULL:preg_last_error(),$pattern);}return$res;}static
function
flatten(array$arr){$res=array();array_walk_recursive($arr,create_function('$a','extract(NCFix::$vars['.NCFix::uses(array('res'=>&$res)).'], EXTR_REFS); $res[] = $a; '));return$res;}}class
NFinder
extends
NObject
implements
...
function
select($masks,$type){$this->cursor=&$this->groups[];$pattern=self::buildPattern($masks);if($type||$pattern){$this->filter(create_function('$file','extract(NCFix::$vars['.NCFix::uses(array('type'=>$type,'pattern'=>$pattern)).'], EXTR_REFS);
				return !$file->isDot()
					&& (!$type || $file->$type())
...
NNRecursiveCallbackFilterIterator($iterator,create_function('$file','extract(NCFix::$vars['.NCFix::uses(array('filters'=>$filters)).'], EXTR_REFS);
...
NNCallbackFilterIterator($iterator,create_function('$file','extract(NCFix::$vars['.NCFix::uses(array('groups'=>$groups)).'], EXTR_REFS);
...
exclude($masks){if(!is_array($masks)){$masks=func_get_args();}$pattern=self::buildPattern($masks);if($pattern){$this->filter(create_function('$file','extract(NCFix::$vars['.NCFix::uses(array('pattern'=>$pattern)).'], EXTR_REFS);
...
new
InvalidArgumentException('Invalid size predicate format.');}list(,$operator,$size,$unit)=$matches;static$units=array(''=>1,'k'=>1e3,'m'=>1e6,'g'=>1e9);$size*=$units[strtolower($unit)];$operator=$operator?$operator:'=';}return$this->filter(create_function('$file','extract(NCFix::$vars['.NCFix::uses(array('operator'=>$operator,'size'=>$size)).'], EXTR_REFS);
...
new
InvalidArgumentException('Invalid date predicate format.');}list(,$operator,$date)=$matches;$operator=$operator?$operator:'=';}$date=NDateTime53::from($date)->format('U');return$this->filter(create_function('$file','extract(NCFix::$vars['.NCFix::uses(array('operator'=>$operator,'date'=>$date)).'], EXTR_REFS);
...
NStaticClassException;}static
function
encode($value){NDebugger::tryError();if(function_exists('ini_set')){$old=ini_set('display_errors',0);$json=json_encode($value);ini_set('display_errors',$old);}else{$json=json_encode($value);}if(NDebugger::catchError($e)){throw
new
...
NDateTime53($t);}else{$value=$t;}$hasValue=TRUE;}}if($inlineParser){if($hasValue){$this->addValue($result,$hasKey,$key,$value);}elseif($hasKey){$this->error();}}else{if($hasValue&&!$hasKey){if($result===NULL){$result=$value;}else{$this->error();}}elseif($hasKey){$this->addValue($result,$key!==NULL,$key,$hasValue?$value:NULL);}}return$result;}private
function
...
as$property){$properties[]=($property->documents?str_replace("\n","\n * ","/**\n".implode("\n",(array)$property->documents))."\n */\n":'').$property->visibility.' $'.$property->name.($property->value===NULL?'':' = '.NPhpHelpers::dump($property->value)).";\n";}return
...
function
split($subject,$pattern,$flags=0){NDebugger::tryError();$res=preg_split($pattern,$subject,-1,$flags|PREG_SPLIT_DELIM_CAPTURE);if(NDebugger::catchError($e)||preg_last_error()){throw
new
NRegexpException($e?$e->getMessage():NULL,$e?NULL:preg_last_error(),$pattern);}return$res;}static
function
match($subject,$pattern,$flags=0,$offset=0){if($offset>strlen($subject)){return
NULL;}NDebugger::tryError();$res=preg_match($pattern,$subject,$m,$flags,$offset);if(NDebugger::catchError($e)||preg_last_error()){throw
new
NRegexpException($e?$e->getMessage():NULL,$e?NULL:preg_last_error(),$pattern);}if($res){return$m;}}static
function
matchAll($subject,$pattern,$flags=0,$offset=0){if($offset>strlen($subject)){return
array();}NDebugger::tryError();$res=preg_match_all($pattern,$subject,$m,($flags&PREG_PATTERN_ORDER)?$flags:($flags|PREG_SET_ORDER),$offset);if(NDebugger::catchError($e)||preg_last_error()){throw
new
NRegexpException($e?$e->getMessage():NULL,$e?NULL:preg_last_error(),$pattern);}return$m;}static
function
...
new
InvalidStateException("Callback '$textual' is not callable.");}foreach((array)$pattern
as$tmp){NDebugger::tryError();preg_match($tmp,'');if(NDebugger::catchError($e)){throw
new
NRegexpException($e->getMessage(),NULL,$tmp);}}$res=preg_replace_callback($pattern,$replacement,$subject,$limit);if($res===NULL&&preg_last_error()){throw
new
NRegexpException(NULL,preg_last_error(),$pattern);}return$res;}elseif($replacement===NULL&&is_array($pattern)){$replacement=array_values($pattern);$pattern=array_keys($pattern);}NDebugger::tryError();$res=preg_replace($pattern,$replacement,$subject,$limit);if(NDebugger::catchError($e)||preg_last_error()){throw
new
NRegexpException($e?$e->getMessage():NULL,$e?NULL:preg_last_error(),$pattern);}return$res;}}class
NRegexpException
extends
Exception{static
...
function
is($value,$expected){foreach(explode('|',$expected)as$item){list($type)=$item=explode(':',$item,2);if(isset(self::$validators[$type])){if(!call_user_func(self::$validators[$type],$value)){continue;}}elseif($type==='number'){if(!is_int($value)&&!is_float($value)){continue;}}elseif($type==='pattern'){if(preg_match('|^'.(isset($item[1])?$item[1]:'').'$|',$value)){return
...
NCallback($callback);}return
new
NCallback(array($callback,$m));}function
